<script lang="ts">
  import { twMerge } from 'tailwind-merge';
  type Props = {
    class?: string;
  };
  const { class: additionalClass }: Props = $props();
  let style = $derived(twMerge('w-16 h-16', additionalClass));
</script>

<svg
  class={style}
  height="800px"
  width="800px"
  xmlns="http://www.w3.org/2000/svg"
  xmlns:xlink="http://www.w3.org/1999/xlink"
  viewBox="0 0 128 128"
  xml:space="preserve"
>
  <path
    id="_x34__44_"
    d="M94.1,20.6L87.8,12L82,24.7L66,30v40l48,16V14L94.1,20.6z M88.2,16l3.9,5.3l-7.5,2.5L88.2,16z M109.5,20.3
	v18.3L95.1,57.9L79.5,34.6l-8.9,16.5V33.3L109.5,20.3z M70.5,66.7v-6l9.3-17.3l15,22.4l14.6-19.6v33.5L70.5,66.7z"
  />
  <path
    id="_x33__48_"
    d="M93,116h-6V86c0-1.1-0.9-2-2-2s-2,0.9-2,2c0,2.2-1,5.3-3,6.9c-1,0.8-2,1-3.1,0.8C72.2,92.7,71,83,71,78
	c0-1.1-0.9-2-2-2s-2,0.9-2,2v30h-6c-1.1,0-2,0.9-2,2s0.9,2,2,2h16c1.1,0,2-0.9,2-2s-0.9-2-2-2h-6V94.2c1.3,1.7,2.9,3,5.1,3.4
	c0.5,0.1,1.1,0.2,1.6,0.2c1.7,0,3.4-0.6,4.8-1.8c0.2-0.1,0.3-0.3,0.5-0.4V116h-6c-1.1,0-2,0.9-2,2s0.9,2,2,2h16c1.1,0,2-0.9,2-2
	S94.1,116,93,116z"
  />
  <ellipse
    id="_x32__75_"
    transform="matrix(0.7071 -0.7071 0.7071 0.7071 -16.2146 50.0137)"
    cx="52.3"
    cy="44.6"
    rx="8"
    ry="8"
  />
  <path
    id="_x31__87_"
    d="M59.8,52.9c-1.8-0.8-3.8-0.1-4.7,1.7L51.2,63l-6.4-11.2c-0.2-0.4-0.5-0.8-0.8-1.2c0,0-0.5-0.6-0.6-0.8
	c-1.8-1.8-4.6-1.8-6.4,0L17.5,69.4c-0.9,0.8-1.5,2.1-1.5,3.4l0,42.2h0c0,2.8,2.2,5,5,5s5-2.2,5-5l0,0l0-34.4c0.7-0.2,1.3-0.4,2-0.8
	c4.8-3,9.4-6.6,13.6-10.8c0.3-0.3,1-1,1.1-1.1c0.8-0.8,1.4-1.6,2-2.5l4.1,7.1c0.3,0.5,0.8,0.9,1.4,1.2c1.8,0.8,3.8,0.1,4.7-1.7
	l6.8-14.5C62.3,55.8,61.6,53.7,59.8,52.9z"
  />
</svg>
