...
const process = async (files, cacheFilepath) => {
  ...
  if (cachedFiles.length) {
    ...
    spinner.prefixText = '';
    spinner.info(
      `${filesDeleted.length} file${filesDeleted.length === 1 ? '' : 's'} ${filesDeleted.length === 1 ? 'was' : 'were'} deleted`
    );
  ...
  if (files.length) {
    ...
    spinner.prefixText = '';
    spinner.info(
      `${filesUploaded.length} file${filesUploaded.length === 1 ? '' : 's'} ${filesUploaded.length === 1 ? 'was' : 'were'} uploaded.`
    );
  }
